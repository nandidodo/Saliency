...
def run_learn_params():
	for i in xrange(num_runs):
		...
		ep = oldep + (lr * (oldphi - oldvp - (oldsp*oldep)))
		eps.append(ep)

		eu = oldeu + (lr * (u - oldphi**2 - (oldsu*oldeu)))
		eus.append(eu)

		vp = oldvp + oldep
		vps.append(vp)

		su = oldsu + 0.5* (oldep**2 - 1/oldsu)
		sus.append(oldsu)
		
		sp = oldsp + 0.5*(oldeu**2 - 1/oldsp)
		sps.append(oldsp)
...
# yeah, I suppose this is a really simple linear dynamical system. it certainly looks that way. I bet it'll get a WHOLE lot more complicated in the brainthough!
